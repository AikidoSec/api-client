...
async function cli(repoId, commitId, options, command) {
    ...
    let pollCount = 1;
    const onStart = () => {
        const scanConfigurationOptions = getScanConfigurationOptions(apiOptions);
        outputLog(chalk.gray(`Scan configuration:\n${scanConfigurationOptions}`));
    ...
    const onFail = (error) => {
        ...
        if (error.response?.status && error.response?.status === 404) {
            outputError('Please verify your commit SHA');
        }
        else {
...
const getScanConfigurationOptions = (apiOptions) => {
    const options = {
